         ...
         public override void Parse(string file)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                switch (indent)
                {
                    ...
                    case 1:
                    case 2:
                        if (indent == 1 && indent > prevIndent) // we know there's a new section because the indent level bumps up
                        {
                            if (false)
                            {
                                // prevLine is e.g., SSID 1 : MyHouseWiFi
                                // colname="SSID" index="1" value="MyHouseWiFi"
                                (l0linename, l0namevalue) = prevLine.SplitColon();
                                (l0colname, l0index) = l0linename.SplitSpace();
                                l0colname = l0colname.Trim();
                                l0namevalue = l0namevalue.Trim();
                        ...
                        // or     BSSID 2                 : 70:3a:cb:19:26:68
                        {
                            ...
                            if (indent == 2 && nextIndent < indent)
                            {
                                ...
                                currRow = savedRows[1]; // I just magically know it's indent level 1 :-)
                            }
                        }
                        break;
                }

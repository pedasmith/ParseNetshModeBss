...
namespace MeCardParser
{
    ...
    public class MeCardParser
    {
        ...
        public static List<MeCardRaw> ParseList(string urlString)
        {
            var retvallist = new List<MeCardRaw>();
            ...
            var schemeSet = scheme.Substring(0, firstColon).ToUpperInvariant();
            retval.Scheme = schemeSet; // should not include the ':'
            retval.SchemeSeperator = ":"; // known because it's what we looked for
            ...
            {
                // is, e.g., S:starpainter
                // Is from having two semicolons in a raw. That will trigger adding in a list
                // entry.
                if (item.Length == 0)
                {
                    if (retval.Fields.Count > 0 || retval.IsValid!=Validity.Valid)
                    {
                        retvallist.Add(retval);
                        retval = new MeCardRaw();
                        retval.Scheme = schemeSet; // should not include the ':'
                        retval.SchemeSeperator = ":"; // known because it's what we looked for
                    }
                }
